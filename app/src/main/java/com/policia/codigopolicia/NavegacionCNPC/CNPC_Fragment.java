    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        try {
            ...
            for (Modelo_ARTICULO articulo : articulos) {
                ...
                String parrafo_numeral = "<p align='justify'>@Nivel &#09; @Numeral <small><a href='javascript:dialogNumeral(@ID)'>(ver procedimiento)</a></small></p>";
                if (!(numerales.size() == 0)) {

                    int pos = 0;
                    do {
                        Modelo_NUMERAL numeral = numerales.get(pos++);
                        html_plantilla_articulos = html_plantilla_articulos.replace("@Numerales", parrafo_numeral.replace("@ID", numeral.ID).replace("@Nivel", numeral.Nivel).replace("@Numeral", numeral.Numeral) + "@Numerales");
